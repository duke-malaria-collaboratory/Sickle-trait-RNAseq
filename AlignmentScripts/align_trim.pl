...
align($GenomeDir, "$trim1", "$trim2", "$out", "$Hs", "$GTF", \@trimReads1, \@trimReads2);
...
sub makeOutDirs
{
    ...
    if ( -d $topOut)
    {
        ...
        unless (-d "$topOut/cuffquant")
        {system "mkdir $topOut/cuffquant";}
        print "\n\tCuffquant files stored in $topOut/cuffquant\n";

        unless (-d "$topOut/cuffnorm")
        {system "mkdir $topOut/cuffnorm";}
        print "\n\tCuffnorm files stored in $topOut/cuffnorm\n";
...
sub align
{
	...
	my $pfalReads2 = "$out/HsUnmappedReads/2/$prefix.2.fastq.gz";
        ...
        system "mv $out/cuffquant/abundances.cxb $out/cuffquant/$prefix.cxb";
    }
}
