...
align($GenomeDir, "$pair1", "$pair2", "$out", "$Hs_genome", \@PairedReadfiles1, \@PairedReadfiles2);
...
sub makeOutDirs
{
    ...
    if ( -d $topOut)
    {
        ...
        unless (-d "$topOut/PRef1UnmappedReads")
        {system "mkdir $topOut/PRef1UnmappedReads";
        system "mkdir $topOut/PRef1UnmappedReads/1";
        system "mkdir $topOut/PRef1UnmappedReads/2";
        system "mkdir $topOut/PRef1UnmappedReads/merged";}
        print "\tUnmapped reads against PRef1 stored in $topOut/PRef1UnmappedReads\n";
...
sub align
{
    ...
    for ($i = 0; $i < $size; $i++)
    {
		...
		my $pfalReads2 = "$out/HsUnmappedReads/2/$prefix.2.fastq.gz";
       	...
       	system "STAR --runMode alignReads --genomeDir $GenomeDir --readFilesIn $pfalReads1 $pfalReads2 --readFilesCommand zcat --outSAMtype BAM Unsorted --outSAMstrandField intronMotif --runThreadN 12 --alignIntronMin 5 --alignIntronMax 1200 --outFilterScoreMinOverLread 0 --outFilterMatchNminOverLread 0  --outFilterMatchNmin 30 --outFilterMismatchNmax 10 --outReadsUnmapped Fastx --outFileNamePrefix $out/STAR_out/Pf_$prefix.";

		system "rm $out/STAR_out/Pf_$prefix.Aligned.out.bam";
		
		system "mv $out/STAR_out/Pf_$prefix.Unmapped.out.mate1 $out/PRef1UnmappedReads/1/$prefix.1.fastq";

		system "mv $out/STAR_out/Pf_$prefix.Unmapped.out.mate2 $out/PRef1UnmappedReads/2/$prefix.2.fastq";

		system "gzip $out/PRef1UnmappedReads/1/$prefix.1.fastq";
		
		system "gzip $out/PRef1UnmappedReads/2/$prefix.2.fastq";
