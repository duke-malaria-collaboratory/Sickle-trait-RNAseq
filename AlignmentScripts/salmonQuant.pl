...
if ($#ARGV != 5) {
    print "usage: Enter full path to target genome directory, output directory, directory with R1 reads, directory with R2 reads, directory with contaminant genome, and gtf file for target genome \ne.g. countRNASeqReads.pl /home/genome /home/out /home/reads/1 /home/reads/2 /home/other_genome /home/gtf";
    exit;
}
my $GenomeDir = $ARGV[0];
my $out = $ARGV[1];
...
sub align
{
    ...
    for ($i = 0; $i < $size; $i++)
    {
		...
		system "salmon quant -i $Hs -l A -1 $read1 -2 $read2 -p 8 --validateMappings -o $out/quant/$prefix";
